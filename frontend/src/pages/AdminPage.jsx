...
import {
  ...
  TrendingUp, Users, Calendar, DollarSign, LogOut, ChevronRight,
  AlertCircle, Eye, CheckCircle, XCircle, Clock, BarChart2
} from 'lucide-react'
...
/* ── Initial demo data ── */
const INIT_CARS = [
  { id:1, name:'Ferrari Roma',        brand:'Ferrari',     category:'Sport',     price:580000, transmission:'Automatique', fuel:'Essence',    seats:2, image:'https://images.unsplash.com/photo-1592198084033-aade902d1aae?w=400&q=80' },
  { id:2, name:'Lamborghini Huracán', brand:'Lamborghini', category:'Sport',     price:790000, transmission:'Automatique', fuel:'Essence',    seats:2, image:'https://images.unsplash.com/photo-1544636331-e26879cd4d9b?w=400&q=80' },
  { id:3, name:'Rolls-Royce Ghost',   brand:'Rolls-Royce', category:'Luxe',      price:640000, transmission:'Automatique', fuel:'Essence',    seats:5, image:'https://images.unsplash.com/photo-1631295868223-63265b40d9e4?w=400&q=80' },
  { id:4, name:'Tesla Model S Plaid', brand:'Tesla',       category:'Électrique',price:210000, transmission:'Automatique', fuel:'Électrique', seats:5, image:'https://images.unsplash.com/photo-1617788138017-80ad40651399?w=400&q=80' },
]

const INIT_BOOKINGS = [
  { id:1, car_name:'Ferrari Roma',        car_brand:'Ferrari',     first_name:'Sophie',    last_name:'Martin',   email:'sophie@email.com',  phone:'+225 07 11 22 33', start_date:'2025-06-10', end_date:'2025-06-13', days:3, total:1740000, status:'confirmed' },
  { id:2, car_name:'Lamborghini Huracán', car_brand:'Lamborghini', first_name:'Alexandre', last_name:'Dupont',   email:'alex@email.com',    phone:'+225 05 55 66 77', start_date:'2025-06-15', end_date:'2025-06-17', days:2, total:1580000, status:'pending'   },
  { id:3, car_name:'Rolls-Royce Ghost',   car_brand:'Rolls-Royce', first_name:'Isabelle',  last_name:'Leclerc',  email:'isa@email.com',     phone:'+225 01 99 88 77', start_date:'2025-06-20', end_date:'2025-06-21', days:1, total:640000,  status:'confirmed' },
  { id:4, car_name:'Tesla Model S Plaid', car_brand:'Tesla',       first_name:'Thomas',    last_name:'Bernard',  email:'thomas@email.com',  phone:'+225 07 44 33 22', start_date:'2025-06-25', end_date:'2025-06-28', days:3, total:630000,  status:'cancelled' },
  { id:5, car_name:'Ferrari Roma',        car_brand:'Ferrari',     first_name:'Marie',     last_name:'Fontaine', email:'marie@email.com',   phone:'+225 05 77 88 99', start_date:'2025-07-01', end_date:'2025-07-04', days:3, total:1740000, status:'pending'   },
]

const INIT_CLIENTS = [
  { id:1, name:'Sophie Martin',    email:'sophie@email.com',  phone:'+225 07 11 22 33', bookings:2, total:2380000, joined:'2025-01-15' },
  { id:2, name:'Alexandre Dupont', email:'alex@email.com',    phone:'+225 05 55 66 77', bookings:1, total:1580000, joined:'2025-02-20' },
  { id:3, name:'Isabelle Leclerc', email:'isa@email.com',     phone:'+225 01 99 88 77', bookings:3, total:2960000, joined:'2025-03-05' },
  { id:4, name:'Thomas Bernard',   email:'thomas@email.com',  phone:'+225 07 44 33 22', bookings:1, total:630000,  joined:'2025-04-12' },
  { id:5, name:'Marie Fontaine',   email:'marie@email.com',   phone:'+225 05 77 88 99', bookings:2, total:3480000, joined:'2025-05-01' },
]
...
export default function AdminPage() {
  ...
  const handleSaveCar = (data) => {
    if (modal === 'add') {
      setCars(prev => [...prev, { ...data, id:nextId, price:Number(data.price), seats:Number(data.seats) }])
      setNextId(v => v+1)
      toast.success('Véhicule ajouté !')
    } else {
      setCars(prev => prev.map(c => c.id === target.id ? { ...c,...data, price:Number(data.price), seats:Number(data.seats) } : c))
  ...
  return (
    <motion.div variants={pageVariants} initial="initial" animate="enter" exit="exit"
      ...
      <aside style={{
        ...
        background:'rgba(12,12,12,0.98)', borderRight:'1px solid rgba(255,255,255,0.05)',
        ...
        <Link to="/" style={{ textDecoration:'none', marginBottom:'36px', display:'block' }}>
          <div style={{ fontFamily:'"Bebas Neue",cursive', fontSize:'1.4rem', letterSpacing:'0.1em', color:'#fff' }}>
            VELOX<span style={{color:'#E8192C'}}>·</span>DRIVE
          </div>
          <div style={{ fontFamily:'"Rajdhani",sans-serif', fontSize:'0.6rem', letterSpacing:'0.25em', color:'rgba(255,255,255,0.2)', textTransform:'uppercase' }}>
            Administration
          </div>
        </Link>
            ...
            return (
              <motion.div key={id} whileHover={{ x:2 }}
                ...
                style={{
                  ...
                  color: active ? '#E8192C' : 'rgba(255,255,255,0.35)',
        ...
        <div style={{ borderTop:'1px solid rgba(255,255,255,0.05)', paddingTop:'16px' }}>
          <div style={{ display:'flex', alignItems:'center', gap:'10px', marginBottom:'12px' }}>
            <div style={{ width:'32px', height:'32px', borderRadius:'50%', background:'linear-gradient(135deg,#E8192C,#B5121F)', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:'"Rajdhani",sans-serif', fontWeight:700, fontSize:'0.8rem', color:'#fff' }}>
        ...
        <AnimatePresence mode="wait">
          ...
          {tab === 'dashboard' && (
            <TabPanel key="dashboard">
                  ...
                  return (
                    <motion.div key={s.label} initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ delay:i*0.07 }}
                      ...
                      <div style={{ fontFamily:'"Bebas Neue",cursive', fontSize:'2rem', letterSpacing:'0.03em', color:'#fff', lineHeight:1 }}>
                        {s.value}{s.suffix && <span style={{ fontSize:'0.55em', color:'rgba(255,255,255,0.4)', marginLeft:'4px' }}>{s.suffix}</span>}
                      </div>
                    </motion.div>
                  )
                })}
              ...
              <BookingsTable bookings={bookings.slice(0,4)} onStatusChange={handleStatusChange} compact/>
          ...
          {tab === 'cars' && (
            <TabPanel key="cars">
              <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:'28px', flexWrap:'wrap', gap:'16px' }}>
                <PageTitle sub={`${cars.length} véhicule${cars.length!==1?'s':''} dans la flotte`}>VÉHICULES</PageTitle>
                        ...
                        <motion.tr key={car.id} initial={{ opacity:0,x:-10 }} animate={{ opacity:1,x:0 }} transition={{ delay:i*0.04 }}
                          ...
                          <td style={{ padding:'14px 16px' }}>
                            <div style={{ display:'flex', alignItems:'center', gap:'10px' }}>
                              <img src={car.image} alt={car.name} style={{ width:'48px', height:'32px', objectFit:'cover', borderRadius:'4px', flexShrink:0 }}/>
          ...
          {tab === 'bookings' && (
            <TabPanel key="bookings">
              ...
              <div style={{ display:'flex', gap:'12px', flexWrap:'wrap', marginBottom:'28px' }}>
                {[
                  { label:'Confirmées', value:bookings.filter(b=>b.status==='confirmed').length, color:'#10B981' },
                  { label:'En attente', value:bookings.filter(b=>b.status==='pending').length,   color:'#F59E0B' },
              ...
              <div style={{ background:'rgba(14,14,14,0.9)', border:'1px solid rgba(255,255,255,0.05)', borderRadius:'12px', overflow:'hidden' }}>
                <div style={{ overflowX:'auto' }}>
                  <table style={{ width:'100%', borderCollapse:'collapse' }}>
                    <thead>
                      <tr style={{ borderBottom:'1px solid rgba(255,255,255,0.05)' }}>
                        {['Client','Email','Téléphone','Réservations','Total dépensé','Depuis'].map(h => (
                          <th key={h} style={{ padding:'12px 16px', textAlign:'left', fontFamily:'"Rajdhani",sans-serif', fontSize:'0.7rem', fontWeight:700, letterSpacing:'0.12em', textTransform:'uppercase', color:'rgba(255,255,255,0.25)', whiteSpace:'nowrap' }}>{h}</th>
                        ))}
                      </tr>
                    </thead>
          ...
          {tab === 'stats' && (
            <TabPanel key="stats">
              ...
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(200px,1fr))', gap:'16px', marginBottom:'36px' }}>
                {[
                  ...
                  <div key={k.label} style={{ padding:'24px', background:'rgba(18,18,18,0.8)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px' }}>
                    ...
                    <div style={{ fontFamily:'"Bebas Neue",cursive', fontSize:'1.8rem', color:k.color, lineHeight:1, letterSpacing:'0.03em' }}>{k.value}</div>
                  </div>
                ))}
              </div>

              {/* Bookings by status bar chart (CSS) */}
                ...
                {[
                  ...
                  <div key={s.label} style={{ marginBottom:'20px' }}>
                    <div style={{ display:'flex', justifyContent:'space-between', marginBottom:'6px' }}>
                      ...
                      <span style={{ fontFamily:'"JetBrains Mono",monospace', fontSize:'0.78rem', color:s.color }}>{s.value} réservation{s.value!==1?'s':''}</span>
                    </div>
                    <div style={{ height:'8px', background:'rgba(255,255,255,0.05)', borderRadius:'4px', overflow:'hidden' }}>
                      <motion.div
                        initial={{ width:0 }}
                        animate={{ width:`${Math.round(s.value/Math.max(bookings.length,1)*100)}%` }}
              ...
              {/* Top cars */}
              <SectionTitle>Véhicules les plus réservés</SectionTitle>
              <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(220px,1fr))', gap:'16px' }}>
                {cars.slice(0,4).map((car,i) => {
                  const carBookings = bookings.filter(b => b.car_name === car.name).length
                  return (
                    <motion.div key={car.id} initial={{ opacity:0,y:20 }} animate={{ opacity:1,y:0 }} transition={{ delay:i*0.08 }}
                      style={{ padding:'20px', background:'rgba(18,18,18,0.8)', border:'1px solid rgba(255,255,255,0.06)', borderRadius:'10px', display:'flex', gap:'14px', alignItems:'center' }}>
                      <img src={car.image} alt={car.name} style={{ width:'60px', height:'42px', objectFit:'cover', borderRadius:'6px', flexShrink:0 }}/>
                      <div>
                        <div style={{ fontFamily:'"Rajdhani",sans-serif', fontSize:'0.92rem', fontWeight:700, color:'#fff', marginBottom:'3px' }}>{car.name}</div>
                        <div style={{ fontFamily:'"JetBrains Mono",monospace', fontSize:'0.75rem', color:'#E8192C' }}>{carBookings} rés.</div>
                      </div>
                    </motion.div>
                  )
                })}
...
/* ── Shared table component for bookings ── */
            ...
            {bookings.map((b,i) => {
              ...
              return (
                <motion.tr key={b.id} initial={{ opacity:0,x:-10 }} animate={{ opacity:1,x:0 }} transition={{ delay:i*0.04 }}
                  ...
                  <Td><span style={{ fontFamily:'"JetBrains Mono",monospace', fontSize:'0.75rem', whiteSpace:'nowrap' }}>{b.start_date} → {b.end_date}</span></Td>
                  <Td>{b.days}j</Td>
                  ...
                  <td style={{ padding:'14px 16px' }}>
                    <div style={{ display:'inline-flex', alignItems:'center', gap:'5px', padding:'4px 10px', background:s.bg, border:`1px solid ${s.border}`, borderRadius:'4px' }}>
                      ...
                      <span style={{ fontFamily:'"Rajdhani",sans-serif', fontSize:'0.72rem', fontWeight:700, letterSpacing:'0.06em', color:s.color, whiteSpace:'nowrap' }}>{s.label}</span>
                    </div>
                  </td>
                  <td style={{ padding:'14px 16px' }}>
                    {b.status !== 'cancelled' && (
                      <div style={{ display:'flex', gap:'6px' }}>
                        {b.status !== 'confirmed' && (
                          <ABtn icon={<CheckCircle size={12}/>} color="#10B981" onClick={()=>onStatusChange(b.id,'confirmed')} title="Confirmer"/>
                        )}
      ...
      <ModalBox title={isEdit?'Modifier le véhicule':'Ajouter un véhicule'} onClose={onClose}>
        <form onSubmit={handleSubmit(onSave)} style={{ display:'flex', flexDirection:'column', gap:'16px' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'16px' }}>
            <MF label="Nom du véhicule" error={errors.name?.message}><input className="form-input" {...register('name',{required:'Requis'})} placeholder="Ferrari Roma"/></MF>
            ...
            <MF label="Places" error={errors.seats?.message}><input className="form-input" type="number" {...register('seats',{required:'Requis',min:{value:1,message:'> 0'}})} placeholder="2"/></MF>
            <MF label="Badge (optionnel)"><input className="form-input" {...register('badge')} placeholder="Nouveau, Top…"/></MF>
          </div>
          <MF label="URL de l'image" error={errors.image?.message}><input className="form-input" {...register('image',{required:'Requis'})} placeholder="https://…"/></MF>
    ...
    <ModalBackdrop onClose={onClose}>
      <ModalBox title="Confirmer la suppression" onClose={onClose}>
        <p style={{ fontFamily:'"DM Sans",sans-serif', fontSize:'0.9rem', color:'rgba(255,255,255,0.5)', marginBottom:'24px', lineHeight:1.6 }}>
          Êtes-vous sûr de vouloir supprimer <strong style={{color:'#fff'}}>{car?.name}</strong> ? Cette action est irréversible.
        </p>
